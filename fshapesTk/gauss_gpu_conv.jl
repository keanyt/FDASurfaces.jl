...
function gauss_gpu_conv_on_device!(TYPE::Type, DIMPOINT::Int, DIMVECT::Int, ooSigma2, x, y, beta, gamma, nx, ny)
    ...
    if i <= nx
        for k = 1:DIMVECT
            gamma[(i-1)*DIMVECT + k] = gammai[k]
        end
    end
end


function gauss_gpu_eval_conv!(TYPE::type, ooSigma2, x_h, y_h, beta_h, gamma_h, dimPoint, dimVect, nx, ny)
    ...
    try
        ...
        if dimPoint == 1 && dimVect == 1
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 1, 1, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
        elseif dimPoint == 2 && dimVect == 1
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 2, 1, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
        elseif dimPoint == 3 && dimVect == 1
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 3, 1, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
        elseif dimPoint == 4 && dimVect == 1
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 4, 1, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
        elseif dimPoint == 2 && dimVect == 2
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 2, 2, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
        elseif dimPoint == 3 && dimVect == 3
            @cuda threads=block_size blocks=grid_size gauss_gpu_conv_on_device!(TYPE, 3, 3, ooSigma2, x_d, y_d, beta_d, gamma_d, nx, ny)
