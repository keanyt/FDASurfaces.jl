function gauss_gpu_grad1_conv_on_device!(TYPE::Type, DIMPOINT::int, ooSigma2, ooSigma4, alpha, x, beta, gamma, nx)
    ...
    betai = Vector{TYPE}(undef, DIMPOINT)
    ...
    if i <= nx  # we will compute gammai only if i is in the range
        ...
        for k = 1:DIMPOINT
            @inbounds betai[k] = beta[(i-1)*DIMPOINT + k]
        end
    end
            ...
            for jrel = 1:blockDim().x
                if jrel <= nx-jstart+1
                    ...
                    for k = 1:DIMPOINT
                        ...
                        for l = 1:DIMPOINT
                            temp = (k==l) ? sdiag : 0
                            t2 = (t1*xmx[l] + temp) * (alphai[l]-alphaj[l])
