...
function sinkhorn_gpu_step(TYPE::Type, epsilon, lambda, weightGeom, weightGrass, 
                            alpha_h, x_h, y_h, beta_h, mu_h, nu_h, gammax_h, gammay_h, gammaWd, gammadx_h, 
    ...
    try
        ...
        elseif dimpoint == 6 && dimVect == 1
            ...
            println("sinkhorn_gpu_conv error: dimensions of grad_xW_on_device kernel not implemented in CUDA")
            CUDA.unsafe_free!(gammadx_d)
            CUDA.unsafe_free!(gammay_d)
            CUDA.unsafe_free!(gammax_d)
            CUDA.unsafe_free!(nu_d)
        ...
        copy!(total_mass + dotpvnu + dotpumu, gammaWd)
