...
function sinkhorn_gpu_step(TYPE::Type, epsilon, lambda, rho, weightGeom, weightGrass, 
                            alpha_h, x_h, y_h, beta_h, mu_h, nu_h, gammax_h, gammay_h, gammaWd, 
    ...
    try
        ...
        for iter = 1:max_iter
            ...
            elseif dimPoint == 8 && dimVect == 1
                ...
                println("sinkhorn_gpu_conv_unbalanced error: dimensions of Sinkhorn kernel not implemented in CUDA")
            ...
            elseif dimpoint == 8 && dimVect == 1
                ...
                println("sinkhorn_gpu_conv_unbalanced error: dimensions of sinkhorn_gpu_grad_conv_on_device kernel not implemented in CUDA")
                CUDA.unsafe_free!(gammay_d)
                CUDA.unsafe_free!(gammax_d)
                CUDA.unsafe_free!(nu_d)
                CUDA.unsafe_free!(mu_d)
        ...
        total_mass = 0.0
        for i = 1:nx
            KLdumu += (expr(-gammax_h[i]/rho)-1) * mu_h[i]
        end
        for j = 1:ny
            KLdvnu += (expr(-gammay_h[i]/rho)-1) * nu_h[j]
        end
        if dimpoint == 2 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 2, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammax_d, nx, ny)
        elseif dimpoint == 3 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 3, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammax_d, nx, ny)
        elseif dimpoint == 4 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 4, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammax_d, nx, ny)
        elseif dimpoint == 6 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 6, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammax_d, nx, ny)
        elseif dimpoint == 8 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 8, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammax_d, nx, ny)
        else
            println("sinkhorn_gpu_conv_unbalanced error: dimensions of Wdual kernel not implemented in CUDA")
            CUDA.unsafe_free!(gammay_d)
            CUDA.unsafe_free!(gammax_d)
            CUDA.unsafe_free!(nu_d)
            CUDA.unsafe_free!(mu_d)
        ...
        # send data from device to host
        Gammax_h = Vector{TYPE}(undef, nx)
        CUDA.copyto!(Gammax_h, gammax_h)

        for i = 1:nx
            total_mass += Gammax_h[i]
        end

        # compute the energy
        # not sure if this actually performs deep copy??
        copy!(total_mass - rho * KLdvnu - rho * KLdumu, gammaWd)
