...
function sinkhorn_gpu_step(TYPE::Type, epsilon, lambda, weightGeom, weightGrass, 
    ...
    try
        ...
        for iter = 1:max_iter
            ...
            elseif dimPoint == 8 && dimVect == 1
                ...
                println("sinkhorn_gpu_conv error: dimensions of Sinkhorn kernel not implemented in CUDA")
            ...
            elseif dimpoint == 8 && dimVect == 1
                ...
                println("sinkhorn_gpu_conv error: dimensions of sinkhorn_gpu_grad_conv_on_device kernel not implemented in CUDA")
                CUDA.unsafe_free!(gammay_d)
                CUDA.unsafe_free!(gammax_d)
                CUDA.unsafe_free!(nu_d)
                CUDA.unsafe_free!(mu_d)
        ...
        dotpumu = dot(gammax_h, mu_h)
        dotpvnu = dot(gammay_h, nu_h)
        total_mass = 0.0

        if dimpoint == 2 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 2, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny)
        elseif dimpoint == 3 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 3, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny)
        elseif dimpoint == 4 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 4, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny)
        elseif dimpoint == 6 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 6, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny)
        elseif dimpoint == 8 && dimVect == 1
            @cuda threads=block_size_y blocks=grid_size_y Wdual_on_device(TYPE, 8, 1, epsilon, weightGeom, weightGrass, alpha_d, x_d, y_d, beta_d, gammadx_d, nx, ny)
        else
            println("sinkhorn_gpu_conv error: dimensions of Wdual kernel not implemented in CUDA")
            CUDA.unsafe_free!(gammay_d)
            CUDA.unsafe_free!(gammax_d)
            CUDA.unsafe_free!(nu_d)
            CUDA.unsafe_free!(mu_d)
        ...
        copy!(gammaWd, total_mass + dotpvnu + dotpumu)
